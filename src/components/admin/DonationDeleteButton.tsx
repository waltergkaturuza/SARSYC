...
interface DonationDeleteButtonProps {
  donationId: string
  label: string
  redirectTo?: string
  variant?: 'icon' | 'button'
...
export default function DonationDeleteButton({
  donationId,
  label,
  redirectTo,
  variant = 'icon',
}: DonationDeleteButtonProps) {
  const router = useRouter()
  const [isDeleting, setIsDeleting] = useState(false)

  const handleDelete = async () => {
    if (!confirm(`Delete donation "${label}"? This cannot be undone.`)) {
      return
    }
